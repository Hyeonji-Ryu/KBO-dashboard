...
class StaticCDN(object):
    ...
    def get_resource_url(self, filename):
        ...
        if self.rev and current_app.config['BOOTSTRAP_QUERYSTRING_REVVING']:
            extra_args['bootstrap5'] = __version__
...
def bootstrap_find_resource(filename, cdn, use_minified=None, local=True):
    ...
    cdns = current_app.extensions['bootstrap5']['cdns']
...
class Bootstrap5(object):
    ...
    def init_app(self, app):
        ...
        local = StaticCDN('bootstrap5.static', rev=True)
        static = StaticCDN()
        ...
        # make flask_bootstrap5
        app.extensions['bootstrap5'] = {
            'cdns': {
                'local': local,
                'static': static,
        ...
        renderers['bootstrap5'] = renderer_name
